...
def processwithcomments(caption, instream, outstream, listingslang = 'C++', hash_script = 'hash-cpp'):
    ...
    nlines = list() # nlines excludes "#inlucde ...", "#pragma once", "using namespace std;" and lines ending with "/** exclude-line */", 
    cur_hash = None
    hash_num = 0
    ...
    # Hash the whole file
    p = subprocess.Popen(['sh', '../content/contest/%s.sh' % hash_script], stdin = subprocess.PIPE, stdout = subprocess.PIPE)
    hsh_whole, _ = p.communicate('\n'.join(nlines))
    hsh_whole = hsh_whole.split(None, 1)[0]
    ...
    nsource_split = nsource.split("\n")
    nsource = ''
    line_num = 0
    
    for line in nsource_split:
        if line_num > 0:
            nsource += '\n'
        line_num += 1
        p = subprocess.Popen(['sh', '../content/contest/%s.sh' % hash_script], stdin = subprocess.PIPE, stdout = subprocess.PIPE)
        hsh_line, _ = p.communicate(line)
        hsh_line = hsh_line.split(None, 1)[0]
        nsource += '(*@{\color{gray}' + hsh_line[0: 2] + '$\,$}@*)' + line
    ...
    if error:
        ...
        if nsource:
            out.append(r"\rightcaption{%s: %s, %d lines}" % (hash_script, hsh_whole, len(nsource.split("\n"))))
        out.append(r"\begin{lstlisting}[caption = {%s}, language = %s]" % (pathescape(caption), listingslang))
        out.append(nsource)
...
def main():
    ...
    try:
        ...
        if language == "cpp" or language == "cc" or language == "c" or language == "h" or language == "hpp":
            processwithcomments(caption, instream, outstream, 'C++', 'hash-cpp')
        elif language == "java":
            processwithcomments(caption, instream, outstream, 'Java', 'hash-java')
        elif language == "ps":
